...
if (!require("visNetwork")) install.packages("visNetwork", repos = "https://cloud.r-project.org")
if (!require("htmlwidgets")) install.packages("htmlwidgets", repos = "https://cloud.r-project.org")
...
# Convert rules to dataframe for plotly
rules_df <- as(rules_0conf, "data.frame")

# Interactive Scatter plot: Support vs Confidence, colored by Lift
p1 <- plot_ly(rules_df, x = ~support, y = ~confidence, color = ~lift,
              text = ~rules, type = "scatter", mode = "markers",
              colors = "RdYlGn", marker = list(size = 8, opacity = 0.7)) %>%
  layout(title = "Support vs Confidence (colored by Lift)",
         xaxis = list(title = "Support"),
         yaxis = list(title = "Confidence"))
p1
saveWidget(p1, "scatter_support_confidence_interactive.html", selfcontained = TRUE)
...
# Interactive Scatter plot: Support vs Lift
p2 <- plot_ly(rules_df, x = ~support, y = ~lift, color = ~confidence,
              text = ~rules, type = "scatter", mode = "markers",
              colors = colorRamp(c("blue", "green", "yellow", "red")), 
              marker = list(size = 8, opacity = 0.7)) %>%
  layout(title = "Support vs Lift (colored by Confidence)",
         xaxis = list(title = "Support"),
         yaxis = list(title = "Lift"))
p2
saveWidget(p2, "scatter_support_lift_interactive.html", selfcontained = TRUE)
...
# ============================================
# TOP 3 RULES BY LIFT (FROM ALL RULES)
# ============================================
...
# Interactive graph (same 3 rules)
p_graph <- plot(top3_rules_graph, method = "graph", engine = "htmlwidget")
p_graph
saveWidget(p_graph, "graph_interactive.html", selfcontained = TRUE)
...
# ============================================
# ADDITIONAL ANALYSIS FOR QUESTIONS j, l
# ============================================
...
cat("  - scatter_support_confidence_interactive.html\n")
cat("  - scatter_support_lift_interactive.html\n")
cat("  - graph_interactive.html (3 rules)\n")
cat("\nDATA FILES:\n")
