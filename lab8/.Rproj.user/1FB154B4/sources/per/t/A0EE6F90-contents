...
barplot(err_income * 100, names.arg = c("10-50K", "50-80K", "GT 80K"),
        ...
        main = "Misclassification by Income Class", ylab = "Error, %",
        ylim = c(0, 100))
...
barplot(err_compare, beside = TRUE, names.arg = c("Female", "Male"),
        col = c("tomato", "dodgerblue"),
        main = "Errors: Before vs After Balancing", ylab = "Error, %",
...
plot(1:5, errors_iter * 100, type = "b", pch = 19, col = "darkgreen",
     ...
     main = "Model Stability", ylim = c(40, 55))
abline(h = mean(errors_iter) * 100, lty = 2, col = "gray")

dev.copy(pdf, "graphs.pdf", width = 10, height = 8)
dev.off()
