...
model_d0 <- suppressWarnings(arima(log_jj, order = c(1, 0, 1), seasonal = list(order = c(0, 1, 1), period = 4)))
...
cat("Comparing final models:\n")
for(i in seq_along(all_models)) {
  cat("  ", names(all_models)[i], ": AIC =", aic_values[i], "\n")
...
cat("Best model:", best_name, "with AIC =", best_aic, "\n")
